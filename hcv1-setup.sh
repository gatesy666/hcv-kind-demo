kubectx kind-hcvault1
# kubectl exec vault-0 -- vault operator init -key-shares=1 -key-threshold=1 -format=json > hcv1-cluster-keys.json
VAULT_UNSEAL_KEY_hcv1=$(cat hcv1-cluster-keys.json | jq -r ".unseal_keys_b64[]")
CLUSTER_ROOT_TOKEN_hcv1=$(cat hcv1-cluster-keys.json | jq -r ".root_token")
...
# kubectl exec --stdin=true --tty=true vault-0 -- vault operator unseal $VAULT_UNSEAL_KEY_hcv1
...
# kubectl exec vault-1 -- vault operator unseal $VAULT_UNSEAL_KEY_hcv1
# kubectl exec vault-2 -- vault operator unseal $VAULT_UNSEAL_KEY_hcv1

# kubectl exec --stdin=true --tty=true vault-0 -- vault login $CLUSTER_ROOT_TOKEN_hcv1
...
vault login $CLUSTER_ROOT_TOKEN_hcv1
...
set|grep hcv1|grep -v BASH_SOURCE
set|grep VAULT_ADDR
